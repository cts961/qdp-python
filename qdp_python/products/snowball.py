...
class Snowball:
    def __init__(self,
        ...
        self.observation_dates = sorted(
            {maturity_date}.union(ki_barrier.observation_dates, ko_barrier.observation_dates))
    ...
    def pv_by_path(self, dates, st, df):
        ...
        for t in range(len(dates)):
            ...
            if self.ko_barrier.is_hit(d, s):
                return self.ko_payoff.pay(s, d) * df[t]
            if self.ki_barrier is not None and ki_flag == BarrierStatus.UnHit and self.ki_barrier.is_hit(d, s):
                ki_flag = BarrierStatus.Hit

        s = st[-1]
        d = dates[-1]
        return (ki_flag * self.ki_payoff.pay(s, d) + (1 - ki_flag) * self.maturity_coupon[d]) * df[-1]
